...
def _fetch_recent_alerts(hours: int = 24) -> list[dict]:
    ...
    try:
        with sqlite3.connect(str(config.DB_PATH)) as conn:
            ...
            cur = conn.execute(
                """
                SELECT grade, coin_symbol, coin_name, news_title, news_url, detected_at, price_at_detection
                FROM news_events
                WHERE grade IN ('S', 'A')
                  AND detected_at >= ?
...
def _ai_commentary(gainers: list[dict], losers: list[dict], picks: list[str]) -> str:
    ...
    picks_str = ", ".join(picks[:10]) if picks else "(없음)"
...
def _format_html(gainers: list[dict], losers: list[dict], alerts: list[dict],
    ...
    picks_html = ", ".join(f"<b>{escape(p)}</b>" for p in picks) if picks else "<i>해당 없음</i>"

    if alerts:
        alerts_html = "<ul>" + "".join(
            f'<li>[{escape(a["grade"])}급] {escape(a["coin_name"])} ({escape(a["coin_symbol"])}) — '
            f'<a href="{escape(a["news_url"])}">{escape(a["news_title"][:60])}</a></li>'
            for a in alerts
        ) + "</ul>"
    else:
