...
SYSTEM_PROMPT = """당신은 가상화폐 뉴스 영향도 평가자입니다. 주어진 한 건의 뉴스 헤드라인+요약이
지정된 종목(symbol)에 대해 정말로 의미있는 호재인지를 판단하고 4단계 등급으로 분류하세요.

[가장 중요한 첫 단계 — 관련성 검증]
종목 매칭이 잘못됐을 수 있어요. 뉴스가 정말 그 종목/프로젝트에 대한 거지 확인하세요.
- 생태계 이름과 코인 이름이 다른 경우 주의 (예: "Cosmos 생태계"는 ATOM 자체와 다름)
- 같은 단어가 다른 의미로 쓰이는 경우 주의 (예: T 토큰 vs 일반 글자 T, 스텔라 코인 vs 게임/회사명)
- 뉴스가 다른 종목에 대한 거면 무관 처리
...
- C: 호재 아님 또는 다른 종목 뉴스. 예) 단순 시세 보도, 다른 종목 끼워팔기, 출처 불명,
     매칭 오류(뉴스가 그 종목에 대한 게 아님).

반드시 JSON으로만 응답하세요. 다른 텍스트는 절대 넣지 마세요.
{"relevant": true|false, "grade": "S|A|B|C", "reason": "한 줄(최대 60자) 한국어 요약"}

relevant=false면 grade는 자동으로 C로 처리됩니다. 매칭 오류로 의심되는 모든 경우 false.
"""
...
def _parse(text: str) -> Classification:
    ...
    # 관련성 false면 강제 C로 다운그레이드 (오탐 알림 방지)
    relevant = data.get("relevant")
    if relevant is False:
        grade = "C"
        reason = f"매칭 무관: {reason}" if reason else "뉴스가 해당 종목과 무관"
