...
업비트 KRW 마켓에서 **2~3일 연속 양봉 종목**을 추리고, **코인니스/토큰포스트 RSS**에서 해당 종목의 호재 뉴스를 발견하면 **Claude Haiku로 S/A/B/C 등급 판정 후 S·A급만 Gmail 알림**을 보내는 봇. 이후 매일 자정(KST) **4가지 종료 기준**으로 사후 추적을 SQLite에 기록.
...
2. **뉴스 수집 + 분류 + 알림** (`app/news.py` → `app/classifier.py` → `app/notifier.py`) — 5분마다 RSS 스캔 → 후보 종목과 매칭되는 호재 발견 → Claude Haiku로 S/A/B/C 등급 → S·A만 이메일 발송
3. **사후 추적** (`app/tracker.py`) — 매일 00:05 KST에 활성 트래킹 4기준 평가:
   - `first_red` (첫 음봉)
...
│  │                                                            │  │
│  │  ┌──────────────────────────────────────────────────────┐  │  │
│  │  │             APScheduler (app/main.py)                │  │  │
│  │  │                                                      │  │  │
│  │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌─────────┐ │  │  │
│  │  │  │price_job │ │news_job  │ │track_job │ │drain_   │ │  │  │
│  │  │  │60분 간격 │ │5분 간격  │ │매일00:05 │ │queue_job│ │  │  │
│  │  │  │          │ │          │ │KST       │ │윈도우4개│ │  │  │
│  │  │  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬────┘ │  │  │
│  │  └───────┼────────────┼────────────┼────────────┼──────┘  │  │
│  │          │            │            │            │         │  │
...
08:30  drain_queue_job 트리거 → pending BTC(A), ETH(S) 일괄 발송 ✉️✉️
        + 각자 price_tracking open
08:35  news_job 정상 동작 — 윈도우 안이므로 새로 발견되면 즉시 발송
...
10:00  국내 윈도우 종료 → 이후 발견되는 S·A는 다시 pending
...
15:30  drain_queue_job 트리거 → 10:00~15:30 사이 쌓인 pending 일괄 발송 ✉️
...
